"""
Tests for RegressionBSDESolver (Gobet-Lemor-Warin).

Covers:
- Accuracy on zero driver / g(x)=x²
- Accuracy on linear driver (vs analytical)
- Quadratic driver vs Cole-Hopf
- Agreement with PicardBSDESolver within MC error
- Basis types, output shapes, reproducibility
- MC std-error ∝ 1/√N
"""
...
class TestRegressionZeroDriver:
    @pytest.fixture
    def setup(self):
    ...
    def test_y0_accuracy(self, setup):
        bsde, exact = setup
        res = RegressionBSDESolver(
            bsde, n_paths=40_000, n_steps=50,
    ...
    def test_output_shapes(self, setup):
        ...
        res = RegressionBSDESolver(
            bsde, n_paths=n_paths, n_steps=n_steps, rng_seed=SEED
        ).solve()
        assert res["Y"].shape     == (n_paths, n_steps + 1)
        assert res["Z"].shape     == (n_paths, n_steps, 1)
        assert res["times"].shape == (n_steps + 1,)
        assert len(res["coeffs"]) == n_steps

    def test_terminal_values_nonneg(self, setup):
        bsde, _ = setup
        res = RegressionBSDESolver(
            bsde, n_paths=2_000, n_steps=30, rng_seed=SEED
        ).solve()
        assert np.all(res["Y"][:, -1] >= 0.0)

    def test_y0_positive(self, setup):
        bsde, _ = setup
        res = RegressionBSDESolver(
            bsde, n_paths=5_000, n_steps=30, rng_seed=SEED
        ).solve()
        assert res["Y0"] > 0.0

    def test_reproducible(self, setup):
        bsde, _ = setup
        y1 = RegressionBSDESolver(bsde, n_paths=500, n_steps=20, rng_seed=9).solve()["Y0"]
        y2 = RegressionBSDESolver(bsde, n_paths=500, n_steps=20, rng_seed=9).solve()["Y0"]
...
class TestRegressionLinearDriver:
    @pytest.fixture
    def setup(self):
    ...
    def test_y0_accuracy(self, setup):
        bsde, exact = setup
        res = RegressionBSDESolver(
            bsde, n_paths=60_000, n_steps=50,
            basis_degree=4, rng_seed=SEED,
        ).solve()
...
class TestRegressionQuadraticDriver:
    @pytest.fixture
    def setup(self):
        ou    = OrnsteinUhlenbeck(kappa=2.0, theta=0.0, sigma=1.0)
        T     = 0.5
    ...
    def test_y0_within_tolerance(self, setup):
        bsde, ref = setup
        res = RegressionBSDESolver(
            bsde, n_paths=30_000, n_steps=60,
            basis_degree=4, rng_seed=SEED,
        ).solve()
...
# ------------------------------------------------------------------
# Agreement between Picard and Regression solvers
# ------------------------------------------------------------------


class TestPicardRegressionAgreement:
    """Both solvers must agree to within MC noise on the same problem."""

    def test_zero_driver_agree(self):
        ou   = OrnsteinUhlenbeck(kappa=2.0, theta=0.0, sigma=1.0)
        bsde = _zero_driver_bsde(ou, T=0.4)

        y_picard = PicardBSDESolver(
            bsde, n_paths=40_000, n_steps=40, n_picard=1,
            basis_degree=4, rng_seed=SEED,
        ).solve()["Y0"]
        y_reg = RegressionBSDESolver(
            bsde, n_paths=40_000, n_steps=40,
            basis_degree=4, rng_seed=SEED,
        ).solve()["Y0"]
        assert abs(y_picard - y_reg) < 0.005, (
            f"Picard={y_picard:.5f} vs Regression={y_reg:.5f}"
        )

    def test_quadratic_driver_agree(self):
        ou    = OrnsteinUhlenbeck(kappa=2.0, theta=0.0, sigma=1.0)
        gamma = 1.0
        h     = lambda x: 0.5 * x**2
        bsde  = StandardBSDE(
        ...
        y_picard = PicardBSDESolver(
            bsde, n_paths=20_000, n_steps=50, n_picard=4,
            basis_degree=4, rng_seed=SEED,
        ).solve()["Y0"]
        y_reg = RegressionBSDESolver(
            bsde, n_paths=20_000, n_steps=50,
            basis_degree=4, rng_seed=SEED,
        ).solve()["Y0"]
        assert abs(y_picard - y_reg) < 0.008, (
            f"Picard={y_picard:.5f} vs Regression={y_reg:.5f}"
        )
...
class TestRegressionBasisTypes:
    @pytest.fixture
    def bsde(self):
        ou = OrnsteinUhlenbeck(kappa=2.0, theta=0.0, sigma=1.0)
...
class TestRegressionMCConvergence:
    def test_std_error_decreases_with_n_paths(self):
        ...
        for n_paths in (2_000, 8_000):
            y0s = [
                RegressionBSDESolver(
                    bsde, n_paths=n_paths, n_steps=30,
